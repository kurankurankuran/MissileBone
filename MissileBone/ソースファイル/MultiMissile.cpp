...
namespace {
	...
	const float g_fRotateSpeed = 2.0f;	// 単位:度
	const float g_fSpeedEnemy = 50.0f;

	const int g_nMaxLife = 600;
	const int g_nDamage = 10;
}



// コンストラクタ
CMultiMissile::CMultiMissile(CScene* pScene) : CWeapon(pScene)
{
	SetID(GOT_MISSILE);
...
// デストラクタ
CMultiMissile::~CMultiMissile()
{
}

// 初期化
HRESULT CMultiMissile::Init()
{
...
void CMultiMissile::Update()
{
	if (!m_bUnit) {
		...
		if (!m_pTarget || !m_pTarget->GetVisible()) {
			...
			case GOT_ENEMY:
				...
				while (pEnemy) {
					if (pEnemy->GetVisible()) {
						...
						if (fZAngle <= 3.14159265359f / 0.5f &&/*dist*/vecTargetLen <= 2500.0f) {
							if (!m_pTarget) {
			...
			// 追跡範囲内（ミサイル前方 90度の円錐以内）にいる場合のみ追跡
			fZAngle = XMVectorGetX(XMVector3AngleBetweenNormals(XMLoadFloat3(&vZ), vecTarget));

			if (fZAngle <= 3.14159265359f / 0.5f && vecTargetLen <= 2500.0f)
			{
				// ターゲットが上下左右どちらか
...
// 発射
int CMultiMissile::Fire(XMFLOAT4X4* pWorld, EGameObjType Type, CModel* pModel)
{
	CScene* pScene = CScene::GetCurrent();
	if (!pScene)
		return -1;
	CMultiMissile* pMissile = new CMultiMissile(pScene);
