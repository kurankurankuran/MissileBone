...
static TPartsInfo g_partsInit[MAX_MISSILE] = 
{
...
CMSManager::CMSManager(CScene* pScene) : CWeaponManager(pScene)
{
...
HRESULT CMSManager::Init(XMFLOAT4X4* pWorld)
{
...
int CMSManager::Create( EGameObjType Type, CModel* pEnemy)
{
	if (!m_pScene)
		return -1;
	...
	for (int i = 0 ; i < m_pWeapon.size(); ++i) {
		...
		if (!m_pWeapon[i]) {
			m_pWeapon[i] = new CMissile(m_pScene);
...
void CMSManager::SetVisible(bool bVisible)
{
	if (m_pParent)
	{
		for (int i = 0; i < MAX_MISSILE; ++i)
