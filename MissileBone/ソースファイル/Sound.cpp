...
void CSound::Init(void)
{
	...
	CSound::SetVolumeBGM(0.1f);
	CSound::SetVolumeSE(0.1f);
}
...
// BGM一括変更
void CSound::SetVolumeBGM(float fVol)
{
	for (int i = 0; i < MAX_BGM; ++i) {
		m_pBgm[i].SetVolume(fVol);
	}
}

// SE一括変更
void CSound::SetVolumeSE(float fVol)
{
	for (int i = 0; i < MAX_SE; ++i) {
		m_pSe[i].SetVolume(fVol);
	}
}
