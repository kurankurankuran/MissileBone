...
// コンストラクタ
CMissile::CMissile(CScene* pScene) : CWeapon(pScene)
{
	SetID(GOT_MISSILE);
...
void CMissile::Update()
{
	if (!m_bUnit) {
		...
		if (!m_pTarget || !m_pTarget->GetVisible()) {
			...
			if (fZAngle <= 3.14159265359f / 4.0f && vecTargetLen <= 10000.0f)
			{
				// ターゲットが上下左右どちらか
		...
		if (m_bVisible && !(m_nEffectCnt % 4)) {
			...
			CEffectManager::StartEffect(EFFECT_SMOKE, EffePos, XMFLOAT2(180.0f, 180.0f), true);
		}
...
// 発射
int CMissile::Fire(XMFLOAT4X4* pWorld, EGameObjType Type, CModel* pModel)
{
